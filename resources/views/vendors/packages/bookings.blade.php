...
@section('content')
  <div class="page-header">
    @if (request()->routeIs('vendor.package_bookings.all_bookings'))
      <h4 class="page-title">{{ __('All Bookings') }}</h4>
    @elseif (request()->routeIs('vendor.package_bookings.paid_bookings'))
      <h4 class="page-title">{{ __('Paid Bookings') }}</h4>
    @elseif (request()->routeIs('vendor.package_bookings.unpaid_bookings'))
      ...
      <div class="card">
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-lg-6">
              <form
                @if (request()->routeIs('vendor.package_bookings.all_bookings')) action="{{ route('vendor.package_bookings.all_bookings') }}"
                @elseif (request()->routeIs('vendor.package_bookings.paid_bookings'))
                  action="{{ route('vendor.package_bookings.paid_bookings') }}"
                @elseif (request()->routeIs('vendor.package_bookings.unpaid_bookings'))
                  action="{{ route('vendor.package_bookings.unpaid_bookings') }}" @endif
            ...
            <div class="col-lg-3">
              <button class="btn btn-danger btn-sm float-right d-none bulk-delete mt-1"
                data-href="{{ route('vendor.package_bookings.bulk_delete_booking') }}"><i
                  ...
                  <table class="table table-striped mt-3">
                    ...
                    <tbody>
                      @foreach ($bookings as $booking)
                          ...
                          <td>
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a href="{{ route('vendor.package_bookings.booking_details', ['id' => $booking->id]) }}"
                                ...
                                <form class="deleteForm d-block"
                                  action="{{ route('vendor.package_bookings.delete_booking', ['id' => $booking->id]) }}"
                                  method="post">
                                  @csrf
                        ...
                        @includeIf('vendors.packages.show_attachment')
                      @endforeach
                    </tbody>
                  </table>
                </div>
  ...
  @includeIf('vendors.packages.send_mail')
@endsection
