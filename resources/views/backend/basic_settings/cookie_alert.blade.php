        ...
        <div class="card-body pt-5 pb-5">
          <div class="row">
            <div class="col-lg-6 offset-lg-3">
              <form id="ajaxForm"
                action="{{ route('admin.basic_settings.update_cookie_alert', ['language' => request()->input('language')]) }}"
                method="post">
                @csrf
                  ...
                  <div class="selectgroup w-100">
                    <label class="selectgroup-item">
                      <input type="radio" name="cookie_alert_status" value="1" class="selectgroup-input"
                        {{ $data != null && $data->cookie_alert_status == 1 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Active') }}</span>
                    </label>

                    <label class="selectgroup-item">
                      <input type="radio" name="cookie_alert_status" value="0" class="selectgroup-input"
                        {{ $data != null && $data->cookie_alert_status == 0 ? 'checked' : '' }}>
                ...
                <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                  ...
                  <input type="text" class="form-control" name="cookie_alert_btn_text"
                    value="{{ $data != null ? $data->cookie_alert_btn_text : '' }}">
                  <p id="err_cookie_alert_btn_text" class="em text-danger mb-0"></p>
                </div>

                <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                  <label for="">{{ __('Cookie Alert Text*') }}</label>
                  <textarea class="form-control summernote" id="cookieSummernote" name="cookie_alert_text" data-height="80">{!! $data != null ? replaceBaseUrl($data->cookie_alert_text, 'summernote') : '' !!}</textarea>
