          ...
          <form id="qrGeneratorForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-lg-6">
                <div class="form-group">
                  @php
                  $qrUrl = !empty($abs->qr_url) ? $abs->qr_url : url('/');
                  @endphp
            ...
            <div id="type-image" class="types">
              <div class="form-group">
                ...
                <div class="col-md-12 showImage mb-3">
                  <img
                    src="{{$abs->qr_inserted_image ? asset('assets/img/qr/' . $abs->qr_inserted_image) :  asset('assets/img/noimage.jpg')}}"
              ...
              <div class="form-group">
                ...
                <input class="form-control p-0 range-slider" name="image_size" type="range" min="1" max="20"
                  value="{{$abs->qr_inserted_image_size}}" onchange="generateQr()">
              ...
              <div class="form-group">
                ...
                <input class="form-control p-0 range-slider" name="image_x" type="range" min="0" max="100"
                  value="{{$abs->qr_inserted_image_x}}" onchange="generateQr()">
                <span class="text-dark size-text float-right">{{$abs->qr_inserted_image_x}}</span>
              </div>

              <div class="form-group">
                <label for="">{{__('Image Vertical Position')}}</label>
                <input class="form-control p-0 range-slider" name="image_y" type="range" min="0" max="100"
                  value="{{$abs->qr_inserted_image_y}}" onchange="generateQr()">
            ...
            <div id="type-text" class="types">
              ...
              <div class="form-group">
                ...
                <input class="form-control p-0 range-slider" name="text_size" type="range" min="1" max="15"
                  value="{{$abs->qr_text_size}}" onchange="generateQr()">
              ...
              <div class="form-group">
                ...
                <input class="form-control p-0 range-slider" name="text_x" type="range" min="0" max="100"
                  value="{{$abs->qr_text_x}}" onchange="generateQr()">
                <span class="text-dark size-text float-right">{{$abs->qr_text_x}}</span>
              </div>

              <div class="form-group">
                <label for="">{{__('Text Vertical Position')}}</label>
                <input class="form-control p-0 range-slider" name="text_y" type="range" min="0" max="100"
                  value="{{$abs->qr_text_y}}" onchange="generateQr()">
    ...
    <div class="col-lg-5">
      <div class="card bg-white">
        <div class="card-header border-bottom">
          ...
          <form action="{{route('admin.qrcode.clear')}}" class="d-inline-block float-right mr-2">
        ...
        <div class="card-body text-center py-5">
          <div class="p-3 border-rounded d-inline-block border bg-light">
            <img id="preview" src="{{asset('assets/img/qr/' . $abs->qr_image)}}" alt="">
          </div>
        </div>

        <div class="card-footer text-center border-top">
          <a id="downloadBtn" class="btn btn-success" download="qr-image.png"
            href="{{asset('assets/img/qr/' . $abs->qr_image)}}">
            {{__('Download Image')}}
          </a>
        </div>
      </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{route('admin.qrcode.save')}}" method="POST" id="qrSaveForm">
            @csrf
            <label for="">{{__('Name')}} **</label>
            <input name="name" type="text" class="form-control" required>
            <p class="text-warning mb-0">{{__('This name will be used to identify this specific QR Code from the QR Codes
              List')}}</p>
          </form>
        </div>
...
@section('script')
  <script type="text/javascript">
    "use strict";
      var qrGenUrl = "{{ route('admin.qrcode.generate') }}";
  </script>
